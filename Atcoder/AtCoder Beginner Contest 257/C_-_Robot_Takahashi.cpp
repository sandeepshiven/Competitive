...
void testcase(){
    ...
    if(child.size() >= adult.size()){

        cout << child.size() + (adult.end() - lower_bound(all(adult), child.back()+1));
    }
    else{
        cout << "adult begin " << *adult.begin() << '\n';
        cout << adult.size() + (lower_bound(all(child), *adult.begin()-1) - child.begin());
    }
